...
class ValidateManager(models.Manager):
    def register(self, postData):
        errorMessages = []
        if len(postData["first_name"]) < 1 or len(postData['last_name']) < 1 or len(postData['password']) < 1 or len(postData['confirm']) < 1 or len(postData['email']) < 1:
            ...
            if len(errorMessages) == 0:
                ...
                newUser = User.objects.create(first_name=postData['first_name'], last_name=postData['last_name'], email=postData['email'], password=hashpw)
                return newUser       
